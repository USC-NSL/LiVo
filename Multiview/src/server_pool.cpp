...
DEFINE_bool(use_server_bitrate_est, true, "Use server side bitrate estimation when mahimahi is used. This will be used only if use_mm=true.");     // Default: Use server side bitrate estimation
    ...
    else 
    {
        try 
        {
            ...
            if (FLAGS_send_ptcl == 0) 
            {
                ...
                MultiviewServerPool serverPool(path, DATASET::PANOPTIC, start_frame_id, end_frame_id, 6);
                serverPool.run();
            }
            else 
            {
                LOG(INFO) << "Run Ptcl pipeline...";
                MultiviewServerPoolPtcl serverPool(path, DATASET::PANOPTIC, start_frame_id, end_frame_id, compression_type, zstd_params, draco_params, 6);
                serverPool.run_ptcl();
            }
        }
        catch(int ex)
...
{
    unique_ptr<WebSocketServer> server = std::make_unique<WebSocketServer>(SERVER_HOST, 8080, FLAGS_send_ptcl);
...
int main(int argc, char** argv)
{
    ...
    if(FLAGS_use_mm)
    {
        ...
        SERVER_HOST = "100.64.0.2";
        
        if(!FLAGS_use_server_bitrate_est && !FLAGS_use_client_bitrate)
            LOG(FATAL) << "One of use_server_bitrate_est or use_client_bitrate should be true when use_mm is true!";   
    }

    if(FLAGS_use_server_bitrate_est && FLAGS_use_client_bitrate)
        LOG(FATAL) << "Both use_server_bitrate_est and use_client_bitrate cannot be true at the same time!";
