// GST_USE_UNSTABLE_API: https://github.com/GStreamer/gst-plugins-bad/blob/ca8068c6d793d7aaa6f2e2cc6324fdedfe2f33fa/gst-libs/gst/webrtc/webrtc_fwd.h#L23
...
// Host webrtc signalling server on localhost
...
void start_pipeline2(websocket::stream<tcp::socket> &ws)
{
	GError *error = NULL;

	const gchar *pipeline_desc = "webrtcbin name=sendonly stun-server=stun://stun.l.google.com:19302 "
                               	"videotestsrc name=source pattern=ball is-live=true ! videoconvert name=convert ! queue name=queue ! vp8enc name=vp8enc ! rtpvp8pay ! "
    							"application/x-rtp, media=video,encoding-name=VP8, payload=96 ! sendonly.";

	pipeline = gst_parse_launch(pipeline_desc,
								&error);
	...
	GstElement *vp8enc = gst_bin_get_by_name(GST_BIN(pipeline), "vp8enc");
	g_object_set(vp8enc, "deadline", 1, NULL);
	gst_object_unref(vp8enc);
...
void handle_websocket_session2(tcp::socket socket)
{
	try
	{
...
void start_pipeline(websocket::stream<tcp::socket> &ws)
{
	GstStateChangeReturn ret;
		...
		// Use videotestsrc instead of v4l2src with is-live=true pattern=ball
		GstElement *v4l2src = gst_element_factory_make("videotestsrc", "source");
		...
		// g_object_set(v4l2src, "device", "/dev/video0", NULL);    // Remove this, since no webcam is connected
		// g_object_set(v4l2src, "pattern", 0, NULL);      // color bars
		g_object_set(v4l2src, "pattern", 18, NULL);     // ball pattern
		g_object_set(v4l2src, "is-live", true, NULL);
		g_object_set(vp8enc, "deadline", 1, NULL);
...
void start_server()
{
	try
	{
		...
		for (;;)
		{
			...
			std::thread{handle_websocket_session2, std::move(socket)}.detach();
		}
	}
	catch (std::exception const& e)
	{
