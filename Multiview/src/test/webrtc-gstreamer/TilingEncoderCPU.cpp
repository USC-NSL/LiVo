...
TilingEncoderCPU::TilingEncoderCPU(const std::string &type, int shape[2], const std::string seq_name, int start_frame_id, int end_frame_id, const std::string &in_dir, const std::string &out_dir, int fps, uint32_t color_bitrate, uint32_t depth_bitrate)
...
TilingEncoderCPU::~TilingEncoderCPU()
{
    if(appsrc != NULL)
    ...
    LOG(INFO) << "TilingEncoderCPU destroyed";
}

void TilingEncoderCPU::start_feed_static(GstElement* appsrc, guint size, gpointer user_data)
{
    auto server = static_cast<TilingEncoderCPU *>(user_data);
...
void TilingEncoderCPU::stop_feed_static(GstElement* appsrc, gpointer user_data)
{
    auto server = static_cast<TilingEncoderCPU *>(user_data);
...
void TilingEncoderCPU::set_appsrc()
{
    if(pipeline == NULL)
    {
    ...
    g_signal_connect(appsrc, "need-data", G_CALLBACK(TilingEncoderCPU::start_feed_static), this);
    g_signal_connect(appsrc, "enough-data", G_CALLBACK(TilingEncoderCPU::stop_feed_static), this);
}

void TilingEncoderCPU::set_appsink()
{
     if(pipeline == NULL)
    {
        g_printerr("Pipeline is not initialized.\n");
...
void TilingEncoderCPU::set_color_pipeline()
{
    GError *g_error = NULL;
...
void TilingEncoderCPU::color_to_buf(const cv::Mat &image, uint8_t *&buf, int &size, const std::string &color_format)
{
    if(image.empty())
    {
...
void TilingEncoderCPU::depth_to_buf(const cv::Mat (&channels)[3], uint8_t *&buf, int &size, const std::string &depth_format)
{
...
bool TilingEncoderCPU::read_color(cv::Mat &image, const int frameID)
{
...
bool TilingEncoderCPU::read_color_untiled(cv::Mat &image, const int frameID, const int viewID)
{
...
bool TilingEncoderCPU::read_depth(cv::Mat (&channels)[3], const int frameID)
{
...
void TilingEncoderCPU::push_untiled()
{
    if(!is_push_buffer_allowed)
    {
        LOG(INFO) << "Push buffer not allowed, waiting for frames ...";
...
GstFlowReturn TilingEncoderCPU::pull_static(GstElement *appsink, gpointer user_data)
{
    auto client = static_cast<TilingEncoderCPU *>(user_data);
    return client->pull();
    // return client->pull_untiled();
}

GstFlowReturn TilingEncoderCPU::pull()
{
    ...
    LOG(INFO) << "Type: " << type << ", Pulled Untiled Frame ID: " << recv_frame_id 
...
GstFlowReturn TilingEncoderCPU::pull_untiled()
{
    GstSample* sample = gst_app_sink_try_pull_sample(GST_APP_SINK(appsink), 10 * GST_MSECOND);
...
void TilingEncoderCPU::end_pipeline()
{
    if(appsrc != NULL)
        gst_object_unref(appsrc);
    appsrc = NULL;
...
void TilingEncoderCPU::start_pipeline()
{
    if(type == "c" || type == "c_bgra")
        set_color_pipeline();
    else
