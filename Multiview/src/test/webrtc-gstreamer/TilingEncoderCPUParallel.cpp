...
TilingEncoderCPUParallel::TilingEncoderCPUParallel(const std::string &type, int shape[2], const std::string seq_name, int start_frame_id, int end_frame_id, int view_id, const std::string &in_dir, const std::string &out_dir, int fps, uint32_t color_bitrate, uint32_t depth_bitrate)
...
TilingEncoderCPUParallel::~TilingEncoderCPUParallel()
{
    if(appsrc != NULL)
    ...
    LOG(INFO) << "TilingEncoderCPUParallel destroyed";
}

void TilingEncoderCPUParallel::start_feed_static(GstElement* appsrc, guint size, gpointer user_data)
{
    auto server = static_cast<TilingEncoderCPUParallel *>(user_data);
    server->is_push_buffer_allowed = true;
    server->push_untiled();
    return;
}

void TilingEncoderCPUParallel::stop_feed_static(GstElement* appsrc, gpointer user_data)
{
    auto server = static_cast<TilingEncoderCPUParallel *>(user_data);
...
void TilingEncoderCPUParallel::set_appsrc()
{
    if(pipeline == NULL)
    {
    ...
    g_signal_connect(appsrc, "need-data", G_CALLBACK(TilingEncoderCPUParallel::start_feed_static), this);
    g_signal_connect(appsrc, "enough-data", G_CALLBACK(TilingEncoderCPUParallel::stop_feed_static), this);
}

void TilingEncoderCPUParallel::set_appsink()
{
     if(pipeline == NULL)
    {
        g_printerr("Pipeline is not initialized.\n");
...
void TilingEncoderCPUParallel::set_color_pipeline()
{
    GError *g_error = NULL;
    ...
    std::string pipeline_desc = "appsrc name=source emit-signals=True do-timestamp=True ! queue ! "
                                ...
                                "x265enc name=videnc ! h265parse ! queue ! appsink name=sink emit-signals=true";
...
void TilingEncoderCPUParallel::color_to_buf(const cv::Mat &image, uint8_t *&buf, int &size, const std::string &color_format)
{
    if(image.empty())
    {
...
void TilingEncoderCPUParallel::depth_to_buf(const cv::Mat (&channels)[3], uint8_t *&buf, int &size, const std::string &depth_format)
{
...
bool TilingEncoderCPUParallel::read_color(cv::Mat &image, const int frameID)
{
...
bool TilingEncoderCPUParallel::read_color_untiled(cv::Mat &image, const int frameID, const int viewID)
{
...
bool TilingEncoderCPUParallel::read_depth(cv::Mat (&channels)[3], const int frameID)
{
...
void TilingEncoderCPUParallel::push_untiled()
{
    ...
    LOG(INFO) << "Type: " << type << ", Pushed Frame ID: " << frame_id << ", View ID: " << view_id;
    ...
    if (frame_id % 100 == 0)
        LOG(INFO) << "Type: " << type << ", Sent Frame: " << frame_id << ", View ID: " << view_id << ", Time taken: " << sw.ElapsedMs() << " ms";
...
GstFlowReturn TilingEncoderCPUParallel::pull_static(GstElement *appsink, gpointer user_data)
{
    auto client = static_cast<TilingEncoderCPUParallel *>(user_data);
    return client->pull_untiled();
}

GstFlowReturn TilingEncoderCPUParallel::pull_untiled()
{
    ...
    int frames_received = recv_frame_id - start_frame_id + 1;
    float avg_time_taken = total_time_taken / frames_received;
    
    LOG(INFO) << "Type: " << type << ", Pulled Untiled Frame ID: " << recv_frame_id << ", View ID: " << view_id
              << ", Time taken: " << time_taken << " ms"
              << ", Avg time: " << avg_time_taken << " ms";
...
void TilingEncoderCPUParallel::end_pipeline()
{
    if(appsrc != NULL)
        gst_object_unref(appsrc);
    appsrc = NULL;
...
void TilingEncoderCPUParallel::start_pipeline()
{
    if(type == "c" || type == "c_bgra")
        set_color_pipeline();
    else
