    ...
    */
    ...
    try 
    {
        ...
        if (FLAGS_send_ptcl == 0)
        {
            ...
            MultiviewClientPool clientPool(path, DATASET::PANOPTIC, start_frame_id, end_frame_id, 5);
            clientPool.run();
            // clientPool.dummy_run();
        }
        else
        {
            LOG(INFO) << "Run Ptcl pipeline...";
            MultiviewClientPoolPtcl clientPool(path, DATASET::PANOPTIC, start_frame_id, end_frame_id, compression_type, zstd_params, draco_params, 4);
            clientPool.run_ptcl();
        }
    }
    catch(int ex)
...
void webrtc_client_worker() 
{
    ...
    if(FLAGS_save_frame)
    {
        string save_frame_path = "/home/lei/data/pipeline/client_tiled/pipeline_new/";
        if(fs::exists(save_frame_path))
            fs::remove_all(save_frame_path);

        if(fs::create_directories(save_frame_path))
            LOG(INFO) << "Received frames will be saved at " << save_frame_path;
        else
            LOG(FATAL) << "Directory creation failed " << save_frame_path;
    }
    
    // // Standalone
    // unique_ptr<WebRTCClientPool> wclient = make_unique<WebRTCClientPool>(path, wdevice_num, FLAGS_tile, FLAGS_save_frame);
    ...
    unique_ptr<WebRTCClientPool> wclient = make_unique<WebRTCClientPool>(wdevice_num, FLAGS_tile, FLAGS_save_frame);
    LOG(INFO) << "WebRTC ClientPool initialized";
    try 
    {
        struct sigaction sa;
        sigemptyset(&sa.sa_mask);
...
{
    unique_ptr<WebSocketClient> client = std::make_unique<WebSocketClient>(SERVER_HOST, 8080, FLAGS_send_ptcl);
