...
DEFINE_string(config_file, "/data/Dropbox/Project/VolumetricVideo/KinectStream/Multiview/config/panoptic.json", "Path to config file.");
...
{
    gflags::SetUsageMessage("Usage: PanopticO3D [OPTION] ...");

    // Parse command line options
    gflags::ParseCommandLineFlags(&argc, &argv, true);
    absl::SetMinLogLevel((absl::LogSeverityAtLeast)FLAGS_log_level);

    parse_config(FLAGS_config_file);
    ...
    client->init(s_deviceIndices, s_deviceSerialNumbers, s_calibrations, false, true);
	...
	try
	{
        ...
        for(uint32_t capture_id = startFrame; capture_id <= endFrame; capture_id++)
        {
            ...
            if(FLAGS_view_ptcl == VIEWER::OPEN3D_VIEWER)
            {
                ...
                // o3d::io::WritePointCloud(FORMAT(output_path << client->get_curr_frameId() << ".ply"), *pcd_ptr);
            ...
            {
                client->collect_stats(FLAGS_collect_stats);
            }
