...
for depth_bitrate = 20000:10000:130000
    depth_bitrate_mbps = depth_bitrate / 1000;

    % toddler4
    base_path = '/datassd/pipeline/client_tiled/pipeline_new/170915_toddler4_with_ground';
    start_frame = 0;
    end_frame = 2000;

    original_path = sprintf('%s/o3d_gt_with_ground_s_nocull_c_nocull/log0/', base_path);
    distorted_path = sprintf('%s/o3d_nvenc_c100m_d_yuv%dm_fps30_s_nocull_c_nocull/', base_path, depth_bitrate_mbps);
    comp_path = sprintf('%s/comp_o3d_pointssim/', base_path);

    if ~exist(comp_path, 'dir')
        mkdir(comp_path);
    end

    out_file_path = sprintf('%s/3D_c100m_d_yuv%dm_pssim.csv', comp_path, depth_bitrate_mbps);
    ...
    pssim_geomBA_list = [];
    pssim_colorBA_list = [];
    for frameID = start_frame:10:end_frame
