...
% compression_method='d_bgra';
% compression_method='d_yuv_unscaled';
compression_method='d_yuv_scaled';
...
distorted_path = sprintf('/datassd/pipeline/client_tiled/ablation/%s/o3d_nvenc_%s_s_pcull_c_cull/%s/log%d/', SEQ_NAME, compression_method, trace_type, logID);
comp_path=sprintf('/datassd/pipeline/client_tiled/ablation/%s/', SEQ_NAME);
out_file_path=sprintf('%s/3D_pssim_%s_%s_%s_log%d.csv', comp_path, SEQ_NAME, compression_method, trace_type, logID);
